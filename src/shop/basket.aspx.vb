...
Partial Class shop_basket
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Init
        If Request.QueryString("empty") = "true" Then Profile.EBCart.emptyBasket()
        ...
        Else
            _currencySign = Profile.EBCart.CurrencySign
        End If
        'EmptyData message must be assigned before gridview is databound
        If siteInclude.trimCrap(getDBResourceString("EmptyBasket")) <> "" Then gvBasket.EmptyDataText = siteInclude.trimCrap(getDBResourceString("EmptyBasket"))
    End Sub
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            bindBasket()
            'If Not basketHasItems() Then lnkProceed.Visible = False
        End If
    End Sub
    ...
    End Sub
    Protected Sub lvBasket_dataBound(ByVal sender As Object, ByVal e As EventArgs)
        If basketHasItems() Then
            ...
            siteInclude.debug("test1")
            For Each row As ListViewItem In lvBasket.Items
                If row.ItemType = ListViewItemType.DataItem Then
                    ...
                    'If the product has a total price of 0.00, then make the quantity non-editable. This will stop customers increasing the qty of free lubes.
                    If lblRowSubTotal.Text = "0.00" Then
    ...
    End Sub
    Protected Sub lnkUpdate_load(ByVal sender As Object, ByVal e As EventArgs)
        Dim tmp As String = ""
        tmp = trimCrap(getDBResourceString("cssUpdate"))
        If tmp <> "" Then CType(sender, LinkButton).CssClass = tmp
        tmp = trimCrap(getDBResourceString("ttUpdate"))
        If tmp <> "" Then CType(sender, LinkButton).ToolTip = tmp
    End Sub
    ...
    Protected Sub btnUpdate_click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        updateShipping()
    End Sub
    Protected Sub lnkEmpty_click(ByVal sender As Object, ByVal e As EventArgs)
        Profile.EBCart.emptyBasket()
    End Sub
    Protected Sub lvBasket_itemUpdating(ByVal sender As Object, ByVal e As ListViewUpdateEventArgs)
        Dim id As Integer = CInt(lvBasket.DataKeys(e.ItemIndex).Value)
        Dim txtQty As TextBox = lvBasket.Items(e.ItemIndex).FindControl("txtQty")
        Profile.EBCart.UpdateItem(CInt(txtQty.Text), id)
        bindBasket()
        updateShipping()
        If Profile.EBCart.Items.Count > 0 Then
            Dim lblTotalIncVat As Label = lvBasket.FindControl("lblTotalIncVat")
            lblTotalIncVat.Text = _currencySign & FormatNumber(Profile.EBCart.TotalInc, 2)
        End If
    End Sub
    Protected Sub lvBasket_rowDeleting(ByVal sender As Object, ByVal e As ListViewDeleteEventArgs)
        bindBasket()
    ...
    Protected Sub gvBasket_rowDeleting(ByVal sender As Object, ByVal e As GridViewDeleteEventArgs)
        ...
        'If Not basketHasItems() Then lnkProceed.Visible = False
    End Sub
    Protected Sub lnkContinueShopping_click(ByVal sender As Object, ByVal e As EventArgs)
        Response.Redirect("~/shopIntro.aspx")
    End Sub
    ...
    Protected Sub btnVoucherSubmit_click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                Dim isPercent As Boolean = CType(dt.Rows(0)("isPercent"), Boolean)
                ...
                Else
                    ...
                    Else
                        ...
                        If bAddToBasket Then
                            ...
                            Profile.EBCart.AddVoucher(txtVoucherNumber.Text, CType(dt.Rows(0)("coupon"), Boolean), credit, isPercent)
                            'Hide voucher boxes
                            'Dim imgBtn As ImageButton = gvBasket.FooterRow.FindControl("btnAddVoucher")
                            'Dim lblPrefix As Label = gvBasket.FooterRow.FindControl("lblVoucherPrefix")
        ...
        End Try
    End Sub
    Protected Sub lnkVoucherSubmit_click(ByVal sender As Object, ByVal e As EventArgs)
        Dim oConn As New SqlConnection(ConfigurationManager.ConnectionStrings("connectionString").ConnectionString)
        Dim oCmd As New SqlCommand("procVoucherByNumberSelect", oConn)
        Dim da As New SqlDataAdapter
        Dim ds As New DataSet
    ...
    Protected Sub btnContinue_click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Dim URL As String = Request.Url.ToString().Replace("basket", "payment")
        If sID > 0 Then
            Profile.EBCart.emptyBasket()
    ...
    Protected Sub bindBasket()
        If Not Profile.EBCart Is Nothing Then
            ...
            gvBasket.DataBind()
            lvBasket.DataSource = Profile.EBCart.Items
            lvBasket.DataBind()
            If lvBasket.Items.Count > 0 Then
                lblCouponVatTotal.Text = _currencySign & lblCouponVatTotal.Text
                lblTotalIncVat.Text = _currencySign & lblTotalIncVat.Text
            End If
        End If
        addShippingToTotal()
    End Sub
    Protected Sub bindVoucher()
        If Profile.EBCart.VoucherNumber <> "" Then
            ...
            panDiscount1.Visible = True
            panDiscount2.Visible = True
            panDiscount3.Visible = True
            'gvBasket
            If False Then
                If Profile.EBCart.VoucherIsCoupon Then
                    ...
                    lblCouponVatTotal.Text = FormatNumber(Profile.EBCart.CouponVat + Profile.EBCart.ShippingVat, 2)
            ...
            If True Then
                If Profile.EBCart.VoucherIsCoupon Then
                    'Show the coupon labels
                    lblDiscount = lvBasket.FindControl("lblCouponDiscount")
                    lblDiscountCost = lvBasket.FindControl("lblCouponDiscountCost")
                    lblDiscountLineBreak = lvBasket.FindControl("lblVoucherLineBreak2")
                    lblCouponVatTotal = lvBasket.FindControl("lblCouponVatTotal")
                    lblVat = lvBasket.FindControl("lblVat")
                    'lblVat.Visible = True
                    lblTotalIncVatCoupon = lvBasket.FindControl("lblTotalIncVatCoupon")
                    lblCouponVatTotal.Visible = True
                    lblTotalIncVatCoupon.Visible = False
                    lblTotalIncVatCoupon.Text = FormatNumber(Profile.EBCart.TotalInc, 2)
                    lblCouponVatTotal.Text = FormatNumber(Profile.EBCart.CouponVat + Profile.EBCart.ShippingVat, 2) & "<br>"
                    'Hide Vat And Other total column
                    gvBasket.Columns(_gvBasket_vatPos).Visible = False
                    gvBasket.Columns(_gvBasket_totalIncPos).Visible = False
                ...
                Else
                    'Show the voucher labels
                    lblDiscount = lvBasket.FindControl("lblVoucherDiscount")
                    lblDiscountCost = lvBasket.FindControl("lblVoucherDiscountCost")
                    lblDiscountLineBreak = lvBasket.FindControl("lblVoucherLineBreak1")
                    'Show Vat And Other total column
                    gvBasket.Columns(_gvBasket_vatPos).Visible = True
                    gvBasket.Columns(_gvBasket_totalIncPos).Visible = True
                End If
            ...
            If (Profile.EBCart.VoucherIsPercent) Then
                lblDiscountCost.Text = "<nobr>" & FormatNumber(Profile.EBCart.VoucherCredit, 2) & "%" & "</nobr>"
            Else
                lblDiscountCost.Text = "<nobr>" & FormatNumber(Profile.EBCart.VoucherCredit, 2) & "</nobr>"
            End If
        Else
            panDiscount1.Visible = False
            panDiscount2.Visible = False
            panDiscount3.Visible = False
            gvBasket.Columns(_gvBasket_vatPos).Visible = True
            gvBasket.Columns(_gvBasket_totalIncPos).Visible = True
        End If
    End Sub
    Protected Sub updateShipping()
        'gvBasket (Delete once dev work is complete)
        If True Then
            Try
            ...
            Catch ex As Exception
                siteInclude.debug("Err1")
            End Try
        End If
        'lvBasket
        If True Then
            Try
                Dim lblShippingTotalCost As Label = lvBasket.FindControl("lblShippingTotalCost")
                Dim lblShippingCost As Label = lvBasket.FindControl("lblShippingCost")
                Dim lblShippingVatRate As Label = lvBasket.FindControl("lblShippingVatRate")
                setShipping(lblShippingCost, lblShippingTotalCost, lblShippingVatRate)
            Catch ex As Exception
                siteInclude.debug("Err2")
            End Try
        End If
    End Sub
    Protected Sub addShippingToTotal()
        If basketHasItems() Then
            Dim lblTotalIncVat As Label
            '
            lblTotalIncVat = gvBasket.FooterRow.FindControl("lblTotalIncVat")
    ...
    Protected Sub testForVoucher()
        ...
        If False Then
            ...
            End If
            lnkProceed.Visible = Not (bVoucherInBasket And bNonVoucherInBasket)
        End If
    ...
    Protected Sub loadDBResources()
        ...
        If gvBasket.Rows.Count > 0 Then
            ...
            lblBasketNote.Text = getDBResourceString("lblBasketNote")
            lblPromoText.Text = getDBResourceString("lblPromoText")
            ...
            'If tmp <> "" Then lnkProceed.ImageUrl = trimCrap(tmp)
            tmp = getDBResourceString("imgUpdateBasket")
            'If tmp <> "" Then lnkUpdate.ImageUrl = trimCrap(tmp)
        ...
        End If
        If lvBasket.Items.Count > 0 Then
            Dim lblShipping As Label = lvBasket.FindControl("lblShipping")
            Dim lblVoucherDiscount As Label = lvBasket.FindControl("lblVoucherDiscount")
            Dim lblVAT As Label = lvBasket.FindControl("lblVAT")
            Dim lblCouponDiscount As Label = lvBasket.FindControl("lblCouponDiscount")
            Dim lblCouponOrderVat As Label = lvBasket.FindControl("lblCouponOrderVat")
            Dim lblTotal As Label = lvBasket.FindControl("lblTotal")
            Dim lblTotalIncVat As Label = lvBasket.FindControl("lblTotalIncVat")
            Dim tmp As String = ""
            'Dim reqTxtVoucherNumber As RequiredFieldValidator = gvBasket.FooterRow.FindControl("reqTxtVoucherNumber")
            'Dim regTxtVoucherNumber As RegularExpressionValidator = gvBasket.FooterRow.FindControl("regTxtVoucherNumber")
            ...
            lblShipping.Text = getDBResourceString("lblShipping")
            lblVoucherDiscount.Text = getDBResourceString("lblVoucherDiscount")
            lblVAT.Text = getDBResourceString("lblVAT") & "<br>"
            lblCouponDiscount.Text = getDBResourceString("lblCouponDiscount")
            lblCouponOrderVat.Text = getDBResourceString("lblCouponOrderVat")
            lblTotal.Text = getDBResourceString("lblTotal")
            lblTotalIncVat.Text = _currencySign & FormatNumber(Profile.EBCart.TotalInc, 2)
            ...
            tmp = trimCrap(getDBResourceString("cssContinueShopping"))
            If tmp <> "" Then lnkContinueShopping.CssClass = tmp
            tmp = trimCrap(getDBResourceString("ttContinueShopping"))
            If tmp <> "" Then lnkContinueShopping.ToolTip = tmp
            tmp = trimCrap(getDBResourceString("cssProceed"))
            If tmp <> "" Then lnkProceed.CssClass = tmp
            tmp = trimCrap(getDBResourceString("ttProceed"))
            If tmp <> "" Then lnkProceed.ToolTip = tmp
            tmp = trimCrap(getDBResourceString("cssSubmit"))
            If tmp <> "" Then lnkVoucherSubmit.CssClass = tmp
            tmp = trimCrap(getDBResourceString("ttSubmit"))
            If tmp <> "" Then lnkVoucherSubmit.ToolTip = tmp
            'rowUpdate button is takencare of in lnkUpdate_load
            'lblTotal.Text = "<b>Total</b>"
            reqTxtVoucherNumber.ErrorMessage = getDBResourceString("errRequired")
            'regTxtVoucherNumber.ErrorMessage = getDBResourceString("errVocuherWrongLength")
            gvBasket.HeaderRow.Cells(_gvBasket_itemPos).Text = getDBResourceString("headerItem")
    ...
    End Function
    'Properties
    Public ReadOnly Property lnkContinueShopping() As LinkButton
        Get
            Return lvBasket.FindControl("lnkContinueShopping")
        End Get
    End Property
    Public ReadOnly Property lnkProceed() As LinkButton
        Get
            Return lvBasket.FindControl("lnkProceed")
        End Get
    End Property
    Protected ReadOnly Property lblTotalIncVat() As Label
        Get
            Return lvBasket.FindControl("lblTotalIncVat")
        End Get
    End Property
    Protected ReadOnly Property lblCouponVatTotal() As Label
        Get
            Return lvBasket.FindControl("lblCouponVatTotal")
        End Get
    End Property
    Public ReadOnly Property panDiscount1() As Panel
        Get
            Return lvBasket.FindControl("panDiscount1")
        End Get
    End Property
    Public ReadOnly Property panDiscount2() As Panel
        Get
            Return lvBasket.FindControl("panDiscount2")
        End Get
    End Property
    Public ReadOnly Property panDiscount3() As Panel
        Get
            Return lvBasket.FindControl("panDiscount3")
        End Get
    End Property
    Public ReadOnly Property lblVAT() As Label
        Get
            Return lvBasket.FindControl("lblVAT")
        End Get
    End Property
End Class
