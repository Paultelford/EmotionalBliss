...
Partial Class affiliates_mediaPictures
    Inherits BasePage
    ...
    Protected Sub showChoice()
        ...
        Select Case LCase(Request.ServerVariables("QUERY_STRING"))
            ...
            Case "llo"
                ...
                directory = "webres"
            Case "logos"
                aFilename = New String() {"EBLogo.tif", "EBText.tif"}
                aFilenamesmall = New String() {"EBLogo", "EBText"}
                aFilesize = New String() {"3.2 MB", "600 kb"}
                directory = "logos"
            Case "julia"
        ...
        If directory <> "" Then
            ...
            For iLoop As Integer = 0 To UBound(aFilename)
                ...
                If directory = "webres" Then
                    tCell.Text = "<img border='1' src='/mediapictures/images/" & directory & "/" & aFilename(iLoop) & "'></a><br>" & aFilesize(iLoop)
                Else
                    tCell.Text = "<a href='/mediapictures/images/" & directory & "/" & aFilename(iLoop) & "'><img border='1' src='/mediapictures/images/" & directory & "/" & aFilenamesmall(iLoop) & "_s.jpg'></a><br>" & aFilesize(iLoop)
                End If
                tCell.BorderWidth = 2
